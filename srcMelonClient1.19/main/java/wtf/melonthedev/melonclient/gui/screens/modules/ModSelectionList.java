...
public class ModSelectionList extends ObjectSelectionList<ModSelectionList.Entry> {
    ...
    public ModSelectionList(HudEditScreen screen, Minecraft minecraft, int width, int height, int y0, int y1, int itemHeight, Supplier<String> searchQuery, ModSelectionList modSelectionList) {
    ...
    protected void moveSelection(AbstractSelectionList.SelectionDirection direction) {
        this.moveSelection(direction, Entry::isSelectable);
    }

    public Optional<ModListEntry> getSelectedOpt() {
        Entry entry = this.getSelected();
        if (entry instanceof ModListEntry modListEntry) return Optional.of(modListEntry);
    ...
    public void updateNarration(NarrationElementOutput output) {
        if (this.children().contains(this.loadingHeader)) this.loadingHeader.updateNarration(output);
        else super.updateNarration(output);
    }
        ...
        for(int j = 0; j < i; ++j) {
            ...
            if (l >= this.y0 && k <= this.y1) {
                ...
                if (true){//this.renderSelection && this.isSelectedItem(j)) {
                    ...
                    GuiUtils.drawHollowRect(this, p_93452_, l1, i1 - 2, k1 - 5, this.itemHeight - 1, Color.LIGHT_GRAY.getRGB());
    ...
    public final class ModListEntry extends Entry implements AutoCloseable {
        ...
        public ModListEntry(ModSelectionList list, ModuleDraggable mod) {
            ...
            this.iconLocation = ResourceLocation.parse("melonclient", "modicons/" + title.toLowerCase());
            this.toggleButton = new Button(getScreen().width/2 + width/2 - 100, y0 + 10, 50, 20, Component.literal("Add"), (button) -> {
            ...
            this.editButton = new Button(getScreen().width/2 + width/2 - 40, y0 + 10, 20, 20, Component.literal("..."), (button) -> {

...
public class MelonScreen extends Screen {
    ...
    public void init() {
        ...
        Client.getInstance().addScreen(this);
        if (drawDoneButton)
            this.addRenderableWidget(new Button(this.width / 2 - 100, this.height / 4 + 132, 200, 20, Component.literal("Done"), button ->
    ...
    public void render(PoseStack stack, int mouseX, int mouseY, float partialTicks) {
        ...
        if (drawMelonString)
            GuiUtils.drawMelonString(this);
        if (drawTitleString) drawCenteredString(stack, this.font, this.title, this.width / 2, drawTitleStringUnderLogo ? 90 : 15, 16777215);
    }
    ...
    public void drawPlayerWithName(PoseStack stack, int x, int y, int scale, float partitialTicks) {
        drawPlayer(x, y, scale, partitialTicks);
        if (player == null) return;
        drawPlayerName(stack,  x - mc.font.width(player.getName()) / 2, y - 130);
    }
    boolean nullplayer;
    ...
    public void drawPlayerName(PoseStack stack, int x, int y) {
        if (player == null) return;
        mc.font.draw(stack, player.getName(), x, y, 0xFFFFFF);
    }


    //Draw Header
    public void drawMelonClientLogoHeader(PoseStack stack, int x, int y) {
        stack.pushPose();
        RenderSystem.setShaderTexture(0, ResourceLocation.parse("textures/gui/title/melonclient.png"));
        RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
        RenderSystem.enableBlend();
        blit(stack, x - 128, y, 0, 0, 128, 64, 128, 128);
        blit(stack, x, y, 0, 64, 128, 64, 128, 128);
        stack.popPose();
        drawString(stack, font, "By Melonthedev                " + Client.getInstance().BASEVVERSION, width / 2 - 62, y + 55, 0xFFFFFF);
    }
}
